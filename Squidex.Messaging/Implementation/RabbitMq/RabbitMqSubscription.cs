...
namespace Squidex.Messaging.Implementation.RabbitMq
{
    internal sealed class RabbitMqSubscription : IMessageAck, IAsyncDisposable
    {
        ...
        public RabbitMqSubscription(string channelName, RabbitMqTransportFactory factory,
            MessageTransportCallback callback, ILogger log)
        {
            ...
            consumerTag = model.BasicConsume(channelName, false, eventConsumer);
            ...
            CancellationToken ct)
        {
            ...
            {
                log.LogError(ex, "Failed to acknowledge message from queue {queue}.", channelName);
            }
            ...
            CancellationToken ct)
        {
            ...
            {
                log.LogError(ex, "Failed to reject message from queue {queue}.", channelName);
            }
